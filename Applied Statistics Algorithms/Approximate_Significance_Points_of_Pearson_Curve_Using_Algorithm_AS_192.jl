...
function pearsn(xbar::Float64, sd::Float64, itype::Int, rb1::Float64, b2::Float64, bndry::Float64)
    # Error codes
    ifault = 0
    ...
    # Coefficients
    a = [-1.9336, -1.6061, 2.6955, -1.7036, 1.7236, -1.3209, -2.5464, 1.6812, -0.11812, 0.050875, 0.16042, -1.0616, 0.43929,
    ...
    if itype == 1
        return compute_significance_points(xbar, sd, itype, rb1, b2, bndry, a, b, rbeta1)
    end

    # Compute B2 using XBAR, SD, RB1, and the known boundary
    if (itype == 2 && xbar <= bndry) || (itype == 3 && xbar >= bndry)
        return ([], 4)
    end
    ...
    if (itype == 2 && (rb1 < bnd1 || rb1 > bnd2)) || (itype == 3 && (rb1 < bnd2 || rb1 > bnd1))
        return ([], 4)
    end
...
function compute_significance_points(xbar::Float64, sd::Float64, itype::Int, rb1::Float64, b2::Float64, bndry::Float64, a::Vector{Float64}, b::Vector{Float64}, rbeta1::Float64)
    # Initialization
    b[1] = rbeta1
    b[2] = b2
    b[3] = rbeta1 * rbeta1
...
function main()
    ...
    sigpt, ifault = pearsn(xbar, sd, itype, rb1, b2, bndry)

    if ifault != 0
        @printf "IFAULT = %d\n" ifault
    else
        @printf "Percentage points\n"
        for pt in sigpt
            @printf " %7.2f" pt
        end
        println()
